...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default='ours') # model name
    ...
    for run in range(opts.num_run):
        ...
        train_set = HSIDataset(image, train_gt, patch_size=opts.patch_size, data_aug=True, use_pca=opts.use_pca, pc=opts.pc, use_mrm=True)
        train_loader = torch.utils.data.DataLoader(train_set, opts.bs, drop_last=False, shuffle=True)

        # load model and loss
        model = get_model(opts.model, opts.dataset_name, pc=opts.pc, num_classes=num_classes, kernel_size=opts.kernel_size, ps=opts.ps, k= opts.k, group_num=opts.group_num, emb_dim=opts.emb_dim)

        # 计算总参数量
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)  # 只计算可训练参数
        ...
        try:
            train(model, optimizer, criterion, criterion2, train_loader, train_loader, opts.epoch, model_dir, device, scheduler)
